---
title: 'Exchange ID Token'
openapi: 'POST /api/v1/auth/exchange'
---

Exchange an ID token from your authentication provider for custom JWT tokens.

This is the primary authentication endpoint. After initial authentication,
clients exchange the ID token for our custom JWT tokens which are used for
...
curl -X POST "https://api.chanl.ai/api/v1/auth/exchange" \
  ...
  -d '{
    "id_token": "eyJhbGciOiJSUzI1NiIsImtpZCI6IjE2NzI5..."
  }'
```
